...
function userErrorHandler($errno, $errmsg, $filename, $linenum, $vars, $type='UNK'){
	if(!empty($errmsg)){
		...
		$logPath = $brunoaw->getPath().'/logs/php';
		...
		if($brunoaw->checkData('user') && $user = $brunoaw->getData('user')){
			if(isset($user->id)){
				$userid = $user->id;
			}
			if(isset($user->name)){
...
function sendMsg(){
	$brunoaw = \Brunoaw::getInstance();
	$msg = "An error has occurred while processing your request, the team has been notified of the problem.";
